...
export const reducer = (state : InitialState ,action : Action) : typeof initialState => {
    switch(action.type) {
        ...
        case "INITIALISE__CURRENT__QUIZ" :
            ...
            const selectedQuiz =  state.attemptedQuestion.filter(( questionObj ) => questionObj.categoryId === quizId ) as Quiz[];
            return {
                ...state,
        ...
        case "SELECT__ANSWER" :
            ...
            if(option === answer) {
                return { ...state, correctAttempt : state.correctAttempt + 1, response : state.response.concat(option)}
            } else {
                return  { ...state, inCorrectAttempt : state.inCorrectAttempt + 1, response : state.response.concat(option)}
            } 
        case "TOTAL__SCORE" : 
            return {
                ...state,
